...
size_t get_size(char *serial_string, size_t *offset) {
	...
	for (i = 0; *offset < strlen(serial_string) && i < INTEGER_DIGITS; *offset += 1, i++) {
		...
		if (serial_string[*offset] >= '0' && serial_string[*offset] <= '9') {
			...
			print_error("Unexpected character in parsing integer");
		}
	}
	*offset += 1;
	...
	while (*offset < strlen(serial_string)) {
		switch (serial_string[*offset]) {
			case 'N':
				...
				else if (have_key && !have_value) {
					*offset += 1;
					entry->value = rson_deserialize_dict(serial_string, offset);
					have_value = true;
				}
				else {
...
rson_object *rson_deserialize_string(char *serial_string, size_t *offset) {
	size_t string_size, serial_string_len;
	char *string;
	rson_object *object;
	...
	if (string_size > strlen(serial_string) - *offset) {
...
rson_object *rson_deserialize(char *serialized_string) {
	...
	// keep eating input until a valid serialization character is read
	for (i = 0; i < strlen(serialized_string); i++) {
		switch (serialized_string[i]) {
			case 'N':
				i++;
			...
			case 'L':
				i++;
				object = rson_deserialize_list(serialized_string, &i);
				break;
			case 'S':
				i++;
				object = rson_deserialize_string(serialized_string, &i);
				break;
			case 'I':
				i++;
				object = rson_deserialize_integer(serialized_string, &i);
				break;
			default:
				continue;
		}
